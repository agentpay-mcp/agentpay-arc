...
If AgentPay tools are unavailable and local command execution is available, ask for explicit approval before running `npx -y @agentpay-ai/agentpay-arc install --runtime claude --mcp-url https://mcp.arc.agentpay.site/mcp`. The install configures a config-free local `agentpay-wallet` MCP; normal users do not need Supabase, RPC, executor, deployer, or bytecode config. Ask them to reload or reconnect the runtime, then return to the agent chat. Use `npx -y @agentpay-ai/agentpay-arc doctor` and `npx -y @agentpay-ai/agentpay-arc setup-web` only for self-hosted/operator diagnostics and fallback. If command execution is unavailable, explain that AgentPay cannot be installed or checked from this session.
