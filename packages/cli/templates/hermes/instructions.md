...
Use AgentPay Arc MCP tools when the human wants to create a Circle Agent Wallet on Arc Testnet, check its budget, send USDC, pay invoices, purchase x402 services, prepare batch payouts, manage agent identity or jobs, or track activity.
...
After installation, continue in chat by calling `setup_agent_wallet` first for the Arc Circle Agent Wallet and follow its returned state. Help the human fund the Arc wallet, check `get_agent_budget`, and use the Arc payment, x402, liquidity, identity, validation, reputation, and job tools requested by the human.

Only for an explicitly requested inherited Celo compatibility workflow, use `prepare_wallet_creation`; for `PENDING`, poll with `check_wallet_creation`, and for `SETUP_REQUIRED`, open `setupUrl`, wait for hosted setup, then call `get_agent_wallet`. Use Review & Sign and the owner EIP-712 path. For that compatibility path, parse invoices with `parse_invoice_payment`, preserve `paymentType`, and when the human provides no URL use `search_x402_services` before `prepare_x402_service_request`. Parse the result with `parse_x402_payment_required`, and call `retry_x402_request` only after `track_payment` returns `COMPLETED`. Read the V2 `PAYMENT-RESPONSE`, include `payment-identifier` idempotency data when advertised, and never execute without a valid owner signature.

For inherited Celo route previews, call `quote_payment_route`, then `prepare_payment`. Use `prepare_contract_call` only for a guarded same-chain contract-call review. Check LI.FI targets with `check_route_target_allowance` and prepare owner changes with `prepare_route_target_allowance`. After a valid owner signature, call `execute_payment`, then `track_payment` and `list_payment_events` before reporting completion.
...
Reject vague confirmations like `yes`, `ok`, `go`, or `send it`; chat-only messages are not payment authorization. On insufficient balance, do not ask for approval or Review & Sign; explain the required top-up.
